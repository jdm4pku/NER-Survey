...
'''
爬取的一些目标会议和期刊
ACL: ccf-a conference
EMNLP: ccf-b conference
COLING: ccf-b conference
NAACL: ccf-b conference
 
TACL: ccf-b journal
CoNLL: ccf-c conference
AACL-IJCNLP: 
LREC:
EACL:
'''
...
for name,url,data in tqdm(urls):
    if data is None:
        t = f"###[{name}]({url})\n\n"
    else:
        t = f"###[{name}]({url}) ({data})\n\n"
    ...
    t_link = "\n#### EL\n"
    t_type = "\n#### Entity Typing\n"
    t_repre = "\n#### Entity Representation / Embedding\n"
    t_others = "\n#### misc\n"
...
with open("scrambled papers.md", "w") as f:
    f.write(txt)
